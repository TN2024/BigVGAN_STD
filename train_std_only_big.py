...
from models_big import BigVGAN, MultiPeriodDiscriminator, MultiResolutionDiscriminator, \
    feature_loss, generator_loss, discriminator_loss, SpectrogramTransformerDiscriminator, transformer_feature_loss
from utils import plot_spectrogram, plot_spectrogram_clipped, scan_checkpoint, load_checkpoint, save_checkpoint, \
    save_audio
...
def train(rank, a, h):
    ...
    # set seed and device
    torch.cuda.manual_seed(h.seed)  # 设置随机种子, 确保复现时排除随机性
    torch.cuda.set_device(rank)  # 目前我们只是用单个 GPU , 因此 rank = 0
    ...
    # define additional discriminators. BigVGAN uses STD as default
    std = SpectrogramTransformerDiscriminator(h).to(device)
    print("Discriminator std params: {}".format(sum(p.numel() for p in std.parameters())))

    # create or scan the latest checkpoint from checkpoints directory
    if rank == rank:
        print(std)
        os.makedirs(a.checkpoint_path, exist_ok=True)
        print("checkpoints directory : ", a.checkpoint_path)  # checkpoint_path exp/bigvgan

    if os.path.isdir(a.checkpoint_path):
        # 检查是否存在最新的checkpoint文件, 如果有, 就返回最新的那一个, 否则返回 None
    ...
    if cp_g is None or cp_do is None:
        ...
        std.load_state_dict(state_dict_do['std'])
    ...
    '''
    mrd 和 mpd 的参数同时被一个优化器更新, 但是他们两个独立拥有自己的权重, 互不干扰, 只是同时更新

    随后我们增加了一个鉴别器, 从此就是三个独立拥有自己的权重, 互不干扰, 只是同时更新
    '''
    optim_g = torch.optim.AdamW(generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
    # optim_d = torch.optim.AdamW(itertools.chain(mrd.parameters(), mpd.parameters(), std.parameters()),
    #                             h.learning_rate, betas=[h.adam_b1, h.adam_b2])
    optim_d = torch.optim.AdamW(std.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
    ...
    '''
    定义了一个调整学习率的方法叫调度器, 作用于优化器 optim_g 和 optim_d, gamma 是学习率衰减系数, 意味着 optim_g 将按照指数衰减的方式调整学习率
    在每个 epoch 结束时, 调度器会使学习率乘以这个衰减系数 'gamma', 从而实现学习率的指数衰减
    '''
    ...
    # unseen_validation_filelist 将包含所看到的训练和验证数据集之外的示例文件路径
    ...
    def validate(rank, a, h, loader, mode="seen"):
        assert rank == rank, "validate should only run on rank=0"
        generator.eval()
        ...
        # 这段代码是使用PyTorch的torchaudio库来创建一个重采样（Resample）变换，目的是将音频数据从一个采样率转换到另一个采样率。
        # 在这个具体的例子中，它创建了一个变换，用于将音频从h.sampling_rate的采样率重采样到16000Hz。此外，这个变换被配置为在CUDA上执行，这意味着它将利用GPU加速计算（如果可用）。
        ...
        with torch.no_grad():
            ...
            for j, batch in tqdm(enumerate(loader)):
                # 梅尔频谱图、音频波形、文件名和为计算损失准备的梅尔频谱图(mel_loss)
                x, y, _, y_mel = batch
                y = y.to(device)
    ...
    if steps != 0 and rank == rank and not a.debug:
        if not a.skip_seen:
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            if rank == rank:
                start_b = time.time()
            # 函数返回处理好的梅尔频谱图、音频波形、文件名和为计算损失准备的梅尔频谱图(mel_loss)。
            x, y, _, y_mel = batch
            ...
            # STD
            y_dt_hat_r, y_dt_hat_g, _, _ = std(y_mel, y_g_hat_mel.detach())
            loss_disc_t, losses_disc_t_r, losses_disc_t_g = discriminator_loss(y_dt_hat_r, y_dt_hat_g)

            # loss_disc_all = loss_disc_s + loss_disc_f + loss_disc_t
            loss_disc_all =  loss_disc_t

            # whether to freeze D for initial training steps
            if steps >= a.freeze_step:
                loss_disc_all.backward()
                # for name, param in std.named_parameters():
                #     if param.grad is not None:
                #         print(name, param.grad)
                #     else:
                #         print(name, "No grad")
                # 对模型参数的梯度进行裁剪，以防止梯度爆炸问题。
                ...
                grad_norm_std = torch.nn.utils.clip_grad_norm_(std.parameters(), 1000.)
                optim_d.step()
            else:
            ...
            # STD loss
            y_dt_hat_r, y_dt_hat_g, fmap_t_r, fmap_t_g = std(y_mel, y_g_hat_mel)
            loss_fm_t = transformer_feature_loss(fmap_t_r, fmap_t_g)
            loss_gen_t, losses_gen_t = generator_loss(y_dt_hat_g)

            if steps >= a.freeze_step:
                # loss_gen_all = loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel + loss_gen_t + loss_fm_t
                loss_gen_all = loss_mel + loss_gen_t + loss_fm_t
            else:
            ...
            if rank == rank:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    fw = jsonlines.open(a.result_file, 'a')
                    result = 'Steps : {:d}, Gen Loss Total : {:4.3f}, Mel-Spec. Error : {:4.3f}, s/b : {:4.3f}'.format(
                        steps, loss_gen_all, mel_error, time.time() - start_b)
                    # result = {'Steps':steps, 'Gen Loss Total':loss_gen_all., 'Mel-Spec. Error ':mel_error, 's/b':time.time() - start_b}
                    fw.write(result)
                    fw.close()
                ...
                if steps % a.checkpoint_interval == 0 and steps != 0:
                    ...
                    save_checkpoint(checkpoint_path,
                                    ...
                                    #  'mrd': (mrd.module if h.num_gpus > 1 else mrd).state_dict(),
                                     'std': (std.module if h.num_gpus > 1 else std).state_dict(),
                ...
                if steps % a.summary_interval == 0:
                    ...
                    sw.add_scalar("training/fm_loss_std", loss_fm_t.item(), steps)
                    sw.add_scalar("training/gen_loss_std", loss_gen_t.item(), steps)
                    sw.add_scalar("training/disc_loss_std", loss_disc_t.item(), steps)
                    sw.add_scalar("training/grad_norm_std", grad_norm_std, steps)
...
def main():
    ...
    parser.add_argument('--list_input_unseen_validation_file', nargs='+',
                        default=['LibriTTS/dev-clean.txt', 'LibriTTS/dev-other.txt'])

    parser.add_argument('--checkpoint_path', default='exp/bigvgan_std_only')
    ...
    parser.add_argument('--result_file', default='exp/result_STD_only.txt')
    ...
    torch.manual_seed(h.seed)  # 设置随机数种子
    '''
    目前我们假设是单cuda运行
    '''
