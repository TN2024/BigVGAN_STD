...
'''
`MAX_WAV_VALUE`在处理音频数据时代表音频波形数据的最大可能值。在数字音频中，波形通常表示为一系列离散的数值，这些数值的范围取决于音频的位深（bit depth）。
例如，对于16位（bit）的PCM（脉冲编码调制）音频，每个样本的可能值范围是从-32768到32767（因为\(2^{15} = 32768\)，16位中一个位用于表示符号）。
在这种情况下，`MAX_WAV_VALUE`可能被设置为32768。
设置`MAX_WAV_VALUE`的目的是在处理音频信号时对其进行规范化，使音频信号的振幅值缩放到-1到1的范围内。
这样的规范化步骤通常是音频处理和深度学习模型输入准备的一个重要环节，因为它有助于模型处理的稳定性和性能。
在上面的代码片段中，通过将音频数据除以`MAX_WAV_VALUE`，实现了这种规范化，使得音频数据的范围标准化到了[-1, 1]内。
这是音频信号预处理中的一个常见步骤，有助于后续的音频分析和处理，比如在转换为梅尔频谱图之前。
'''
...
def get_dataset_filelist(a):
    with open(a.input_training_file, 'r', encoding='utf-8') as fi:  #a.input_training_file = LibriTTS/train-full.txt
        training_files = [os.path.join(a.input_wavs_dir, x.split('|')[0] + '.wav')  #a.input_wavs_dir = LibriTTS
...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            '''
            数字音频的波形实际上是一系列离散的数值, 因此这里的 audio 也是一组离散值
            '''
            audio, sampling_rate = load_wav(filename, self.sampling_rate)
            audio = audio / MAX_WAV_VALUE   #规范化之后在 {-1, 1}之间的离散值
